...
void OneBlockScript::Start()
{
    assert(GET_SINGLE(BlockTable)->IsLoaded() &&
        "OneBlockScript::Start() 전에 BlockTable::Load() 가 선행되어야 합니다.");

    const auto& phaseTable = GET_SINGLE(BlockTable)->GetPhaseSequence();
...
void OneBlockScript::Mine()
{
    ...
    if (_pInventory)
        _pInventory->AddItem(GetCurrentDropSlotType(), 1);
}
