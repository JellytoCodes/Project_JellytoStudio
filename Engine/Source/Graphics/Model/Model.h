...
public:
	...
	ModelBone*										GetBoneByIndex(uint32 index)							{ return (index >= _bones.size()) ? nullptr : _bones[index].get(); }
	...
	ModelAnimation*									GetAnimationByIndex(uint32 index)						{ return (index >= _animations.size()) ? nullptr : _animations[index].get(); }
