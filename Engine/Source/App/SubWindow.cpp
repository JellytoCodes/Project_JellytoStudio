...
bool SubWindow::Create(HINSTANCE hInstance, HWND hMainWnd,
...
void SubWindow::SwitchPanel(ActivePanel panel)
{
...
void SubWindow::BuildFbxConverterPanel()
{
...
void SubWindow::OnBrowseModelFbx()
{
...
void SubWindow::OnBrowseAnimFbx()
{
	wchar_t buf[MAX_PATH] = {};
	OPENFILENAMEW ofn = {};
	ofn.lStructSize = sizeof(ofn);
...
bool SubWindow::RunConvertModel(const std::wstring& fbxPath,
...
bool SubWindow::RunConvertAnim(const std::wstring& fbxPath,
	const std::wstring& outputDir, const std::wstring& baseName)
{
	try
	{
...
void SubWindow::SetStatus(const std::wstring& msg, bool success)
{
...
void SubWindow::BuildModelBrowserPanel()
{
	auto C = [&](const wchar_t* cls, const wchar_t* txt, DWORD s,
		int x, int y, int w, int h, int id) -> HWND
	{
...
void SubWindow::RefreshModelList()
{
...
void SubWindow::RegisterWindowClass(HINSTANCE hInstance)
{
	WNDCLASSEXW wcex   = {};
...
LRESULT CALLBACK SubWindow::WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	if (msg == WM_NCCREATE)
	{
	...
	SubWindow* self = reinterpret_cast<SubWindow*>(::GetWindowLongPtr(hWnd, GWLP_USERDATA));
	if (self)
	{
		switch (msg)
		{
