...
void Scene::Render()
{
	...
	for (Entity* object : _widgetObjects)
	{
...
void Scene::Add(std::unique_ptr<Entity> object)
{
	...
	if (dynamic_cast<Widget*>(object.get()))
		_widgetObjects.push_back(object.get());
...
void Scene::Remove(Entity* object)
{
	...
	_widgetObjects.erase(
		std::remove(_widgetObjects.begin(), _widgetObjects.end(), object),
		_widgetObjects.end());
...
Entity* Scene::Pick(int32 screenX, int32 screenY)
{
	...
	for (auto& entity : _objects)
	{
		...
		{
			minDist = dist;
			picked  = entity.get(); // raw pointer — Scene이 소유
		}
	}
